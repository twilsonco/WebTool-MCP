...
```bash
uv run python examples/run_examples.py fetch      # web_fetch demos only
uv run python examples/run_examples.py search     # web_search demos only
uv run python examples/run_examples.py summarize  # web_summarize demos only
uv run python examples/run_examples.py all        # everything in sequence
```

### test_mcp.py

End-to-end MCP client test that connects via the streamable-http transport and calls all three tools through the MCP protocol (JSON-RPC). Tests `web_search` with each provider (miklium, tavily, brave, google), `web_fetch`, and `web_summarize`.
